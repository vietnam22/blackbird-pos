...
function FloorView({ 
  ...
  const handleCreditorsClick = () => {
    // Don't mark as seen here - let CreditorView handle it on exit
    onCreditorsClick()
  }
  ...
  // Reload notifications when completedBills changes (e.g., returning from another view)
  useEffect(() => {
    loadNotifications()
  }, [completedBills])
  ...
  // Replace the loadNotifications function in FloorView.jsx with this:

const loadNotifications = async () => {
  // Load pending inventory requests (admin only)
  if (user.role === 'admin') {
  ...
  // Load credit activity count (admin only)
  // Count: 1) Pending clear requests, 2) New activity since last visited
  if (user.role === 'admin') {
    const lastSeen = getLastSeenCreditors()
    
    // Helper to check if bill is cleared (handles both property names)
    const isBillCleared = (b) => b.creditPaid === true || b.creditCleared === true
    
    // Get credit logs to count new activity
    const logsData = await api.getCreditLogs()
    const logs = logsData.logs || []
    
    // Count new logs since last seen (but not pending clear requests - we count those separately)
    const newLogsSinceLastSeen = logs.filter(log => {
      const logTime = new Date(log.timestamp)
      return logTime > lastSeen && log.type !== 'clear_requested'
    }).length
    
    // Count pending clear requests that need approval (always show these)
  ...
  const todayBills = completedBills.filter(
    b => new Date(b.timestamp) >= todayStart
  )
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.navBar}>
        <button onClick={onSalesClick} style={styles.navButton}>
          📊 Sales
        </button>
        ...
        <button onClick={onStaffClick} style={styles.navButton}>
          ⏱️ Clock
        </button>
        {user.role === 'admin' && (
          <button onClick={onAdminClick} style={styles.navButton}>
            ⚙️ Admin
          </button>
        )}
      </div>
